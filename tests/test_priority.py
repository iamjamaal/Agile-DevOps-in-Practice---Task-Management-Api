...
class TestTaskPriority:
    ...
    def test_create_task_with_low_priority(self):
        ...
        response = client.post('/tasks', json={'title': 'Low priority task', 'priority': 'low'})
    ...
    def test_create_task_with_high_priority(self):
        ...
        response = client.post('/tasks', json={'title': 'High priority task', 'priority': 'high'})
    ...
    def test_create_task_with_invalid_priority(self):
        ...
        response = client.post(
            '/tasks', json={'title': 'Invalid priority task', 'priority': 'urgent'}
        )
    ...
    def test_update_task_priority(self):
        ...
        update_response = client.patch(f'/tasks/{task_id}', json={'priority': 'high'})
    ...
    def test_update_priority_from_high_to_low(self):
        ...
        update_response = client.patch(f'/tasks/{task_id}', json={'priority': 'low'})
    ...
    def test_priority_persists_across_status_update(self):
        ...
        # Create high priority task
        create_response = client.post('/tasks', json={'title': 'Test task', 'priority': 'high'})
        task_id = create_response.json()['id']

        # Update status only
        update_response = client.patch(f'/tasks/{task_id}', json={'status': 'completed'})
    ...
    def test_all_priority_levels(self):
        ...
        for priority in priorities:
            response = client.post(
                '/tasks',
                json={'title': f'{priority.capitalize()} priority task', 'priority': priority},
            )
